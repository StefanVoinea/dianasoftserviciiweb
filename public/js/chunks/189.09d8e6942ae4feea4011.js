  ...
  methods: {
    ...
    incarca: function incarca() {
      ...
      this.$http.post('/declaratii', formular, {
        ...
        _this4.notifica(parti.join(', '), respinse || erori.length ? 'warning' : 'success');

        if (erori.length) {
          _this4.eroare = erori.join(' | ');
        ...
        return _this4.semneazaValidate(incarcate.filter(function (d) {
          return d.pas === 'validat';
        }), incarcate.filter(function (d) {
          return d.pas === 'semnat';
        }));
      })["catch"](function (err) {
        var date = err.response && err.response.data;
        _this4.eroare = date && date.erori && date.erori.length ? date.erori.join(' | ') : _this4.mesajEroare(err, 'Nu s-au putut încărca declarațiile');
      })["finally"](function () {
        _this4.incarcaInCurs = false;
    ...
    semneazaValidate: function semneazaValidate(declaratii) {
      ...
      var urmatoarea = function urmatoarea(i) {
        if (i >= declaratii.length) {
          if (esuate.length) {
            _this5.eroare = "Nu s-au putut semna: ".concat(esuate.join(' | '));
          }

          _this5.notifica("".concat(declaratii.length - esuate.length, " din ").concat(declaratii.length, " semnate"), esuate.length ? 'warning' : 'success');

          return null;
        }

        var declaratie = declaratii[i];
        _this5.semnareInCurs = "".concat(declaratie.tip, " (").concat(i + 1, " din ").concat(declaratii.length, ")");
        return _this5.$http.post("/declaratii/".concat(declaratie.id, "/semneaza")).then(function () {
          reusite.push(declaratie);
        })["catch"](function (err) {
          esuate.push("".concat(declaratie.tip, " ").concat(declaratie.cui, ": ").concat(_this5.mesajEroare(err, 'semnare eșuată')));
        }).then(function () {
      ...
      return urmatoarea(0).then(function () {
        _this5.semnareInCurs = ''; // Se adună doar cele semnate acum, nu tot ce e semnat în tabel.

        if (_this5.tiparire && reusite.length) {
          _this5.semnareInCurs = 'fișierul pentru tipărire';
          return _this5.descarcaPentruTiparire(reusite.map(function (d) {
            return d.id;
          })).then(function () {
            _this5.semnareInCurs = '';
          });
        }

        return null;
      }).then(function () {
        return _this5.depuneDinSesiune([].concat(Object(C_dianasoft_serviciiweb_web_node_modules_babel_runtime_helpers_esm_toConsumableArray_js__WEBPACK_IMPORTED_MODULE_1__["default"])(venitesemnate), reusite));
      });
    },
    ...
    descarcaPentruTiparire: function descarcaPentruTiparire(ids) {
      ...
      return this.$http.post('/declaratii/concateneaza', Object(C_dianasoft_serviciiweb_web_node_modules_babel_runtime_helpers_esm_objectSpread2_js__WEBPACK_IMPORTED_MODULE_0__["default"])(Object(C_dianasoft_serviciiweb_web_node_modules_babel_runtime_helpers_esm_objectSpread2_js__WEBPACK_IMPORTED_MODULE_0__["default"])({}, date), {}, {
        ...
        _this6.$bvToast.toast("".concat(data.data.documente, " documente trimise la \u201E").concat(data.data.imprimanta, "\u201D."), {
        ...
        _this6.eroare = "".concat(ceAnume, " sunt gata, dar tip\u0103rirea nu a reu\u0219it: ").concat(motiv, " Se descarc\u0103 fi\u0219ierul.");
        return _this6.descarcaFisierUnit(date, ids.length);
      });
    },
    ...
    descarcaFisierUnit: function descarcaFisierUnit(date, cate) {
      ...
      return this.$http.post('/declaratii/concateneaza', date, {
        ...
        _this7.eroare = 'Nici fișierul pentru tipărire nu a putut fi creat.';
      });
    },
    ...
    depuneSemnate: function depuneSemnate() {
      ...
      this.$bvModal.msgBoxConfirm("Se trimit acum la ANAF ".concat(numite, " (").concat(listate, "). Dup\u0103 trimitere nu mai pot fi retrase."), {
        ...
        return _this8.trimiteLaAnaf(declaratii);
      });
    },
    ...
    trimiteLaAnaf: function trimiteLaAnaf(declaratii) {
      ...
      var urmatoarea = function urmatoarea(i) {
        if (i >= declaratii.length) {
          if (esuate.length) {
            _this9.eroare = "Nu s-au putut depune: ".concat(esuate.join(' | '));
          }

          _this9.notifica("".concat(declaratii.length - esuate.length, " din ").concat(declaratii.length, " depuse"), esuate.length ? 'warning' : 'success');

          return null;
        }

        var declaratie = declaratii[i];
        _this9.depunereMesaj = "".concat(declaratie.tip, " (").concat(i + 1, " din ").concat(declaratii.length, ")");
        return _this9.$http.post("/declaratii/".concat(declaratie.id, "/depune"))["catch"](function (err) {
          esuate.push("".concat(declaratie.tip, " ").concat(declaratie.cui, ": ").concat(_this9.mesajEroare(err, 'depunere eșuată')));
        }).then(function () {
          return urmatoarea(i + 1);
        });
      };
      ...
      return urmatoarea(0).then(function () {
        _this9.depunereInCurs = false;
        _this9.depunereMesaj = '';
    ...
    actiune: function actiune(declaratie, tip) {
      ...
      this.$http.post("/declaratii/".concat(declaratie.id, "/").concat(tip)).then(function () {
        _this10.notifica(mesaje[tip], 'success');

        _this10.incarcaLista();
      })["catch"](function (err) {
        _this10.eroare = _this10.mesajEroare(err, 'Operația a eșuat');
    ...
    verificaRecipise: function verificaRecipise() {
      ...
      this.$http.post('/declaratii/recipise').then(function (raspuns) {
        ...
        _this11.ultimaDescarcare = _this11.acum();
        _this11.ultimaDescarcareNumar = rezultat.descarcate || 0;
        window.localStorage.setItem('declaratii_recipise_ultima', JSON.stringify({
          la: _this11.ultimaDescarcare,
          descarcate: _this11.ultimaDescarcareNumar
        }));

        if (!tacut || rezultat.descarcate > 0) {
          _this11.notifica("Verificate: ".concat(rezultat.verificate, ", recipise desc\u0103rcate: ").concat(rezultat.descarcate), rezultat.descarcate > 0 ? 'success' : 'info');
        }

        if (rezultat.erori && rezultat.erori.length) {
          _this11.eroare = rezultat.erori.join(' | ');
        }

        _this11.incarcaLista(); // Recipisele venite acum se pot aduna într-un singur fișier de tipărit.


        var aduse = rezultat.descarcate_id || [];

        if (_this11.tiparireRecipise && aduse.length) {
          return _this11.descarcaPentruTiparire(aduse, 'recipisa');
        }

        return null;
      })["catch"](function (err) {
        _this11.eroare = _this11.mesajEroare(err, 'Descărcarea recipiselor a eșuat');
      })["finally"](function () {
        _this11.recipiseInCurs = false;
      });
    },
    sterge: function sterge(declaratie) {
      ...
      this.$bvModal.msgBoxConfirm("\u0218terge\u021Bi declara\u021Bia ".concat(declaratie.tip, " pentru CUI ").concat(declaratie.cui, "?"), {
        ...
        _this12.$http["delete"]("/declaratii/".concat(declaratie.id)).then(function () {
          _this12.notifica('Declarația a fost ștearsă', 'success');

          _this12.incarcaLista();
        })["catch"](function (err) {
          _this12.eroare = _this12.mesajEroare(err, 'Ștergerea a eșuat');
        });
      });
    },
    mesajEroare: function mesajEroare(err, implicit) {
    ...
    explicaEroarea: function explicaEroarea(declaratie) {
      ...
      fetch(adresa, {
        ...
        return _this13.citesteFluxul(raspuns.body.getReader());
      })["catch"](function (err) {
        // Eșecul se arată în fereastră, nu în spatele ei: altfel utilizatorul
        // vede doar o fereastră goală și nu știe ce s-a întâmplat.
        _this13.explicatieInCurs = false;
        _this13.explicatieEroare = "Explica\u021Bia nu a putut fi ob\u021Binut\u0103 (".concat(err.message, ").");
      });
    },
    ...
    citesteFluxul: function citesteFluxul(cititor) {
      ...
      var urmatorul = function urmatorul() {
        return cititor.read().then(function (_ref2) {
          var done = _ref2.done,
              value = _ref2.value;

          if (done) {
            _this14.explicatieInCurs = false;
            return null;
          }
