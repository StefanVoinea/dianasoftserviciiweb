...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    solicita: function solicita() {
      ...
      this.$http.post('/anaf-societati/solicita').then(function (raspuns) {
        var r = raspuns.data.data;
        var parti = [];
        if (r.trimise) parti.push("".concat(r.trimise, " solicit\u0103ri trimise"));
        if (r.reinterpretate) parti.push("".concat(r.reinterpretate, " documente reinterpretate"));
        if (r.sarite) parti.push("".concat(r.sarite, " s\u0103rite (deja cerute azi sau persoane fizice)"));
        _this4.info = parti.length ? parti.join(', ') : 'Nu era nimic de solicitat.';

        if (r.erori && r.erori.length) {
          _this4.eroare = r.erori.join(' | ');
        }
    ...
    salveaza: function salveaza() {
      ...
      this.$http.put("/anaf-societati/".concat(this.formular.id), {
        ...
        _this5.eroare = _this5.mesajEroare(err, 'Salvarea a eșuat');
      });
    },
    mesajEroare: function mesajEroare(err, implicit) {
      return err.response && err.response.data && err.response.data.message ? err.response.data.message : implicit;
...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    salveaza: function salveaza() {
      ...
      this.eroareFormular = '';
      var cerere = this.formular.id ? this.$http.put("/client/utilizatori/".concat(this.formular.id), this.formular) : this.$http.post('/client/utilizatori', this.formular);
      cerere.then(function () {
                    ...
                    _c(
                      "b-button",
                      {
                        attrs: {
                          ...
                          disabled: !_vm.deDepus.length || _vm.depunereInCurs,
                        },
                ...
                fn: function (rand) {
                  return [
                    rand.item.pas === "eroare_semnare"
                      ...
                      : _vm._e(),
                    _vm._v(" "),
                    rand.item.semnat && !rand.item.index_recipisa
                      ? _c(
                          "b-button",
                          {
                            staticClass: "mr-1 mb-1",
