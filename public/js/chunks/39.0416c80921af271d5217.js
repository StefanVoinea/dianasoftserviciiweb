  ...
  methods: {
    ...
    incarcaLista: function incarcaLista() {
      ...
      this.$http.get('/anaf-certificate').then(function (raspuns) {
        ...
        _this.versiuneProgram = raspuns.data.versiune_program || '';
      })["catch"](function (err) {
    ...
    comutaActiv: function comutaActiv(certificat) {
      ...
      this.$http.post("/anaf-certificate/".concat(certificat.id, "/activare")).then(function (_ref3) {
        ...
        if (!data.data.activ && _this7.certificatActiv === certificat.id) {
          _this7.certificatActiv = null;
          delete _this7.$http.defaults.headers.common['X-Certificat-Id'];
          window.localStorage.removeItem('anaf_certificat_activ');
        }

        _this7.incarcaLista();
      })["catch"](function (err) {
        _this7.eroare = _this7.mesajEroare(err, 'Starea certificatului nu a putut fi schimbată');
      })["finally"](function () {
        _this7.activareInCurs = null;
      });
    },
    ...
    ataseaza: function ataseaza() {
      ...
      this.$http.post("/anaf-certificate/".concat(this.certificatCurent.id, "/utilizatori"), this.utilizatorNou).then(function () {
        ...
        _this8.eroareModal = _this8.mesajEroare(err, 'Utilizatorul nu a putut fi atașat');
      });
    },
    detaseaza: function detaseaza(utilizator) {
      ...
      this.$http["delete"]("/anaf-certificate/utilizatori/".concat(utilizator.id)).then(function () {
        ...
        _this9.eroareModal = _this9.mesajEroare(err, 'Utilizatorul nu a putut fi eliminat');
      });
    },
    ...
    reincarcaSiActualizeazaModalul: function reincarcaSiActualizeazaModalul() {
      ...
      this.$http.get('/anaf-certificate').then(function (raspuns) {
        _this10.certificate = raspuns.data.data || [];
        _this10.abonati = raspuns.data.abonati || [];
        _this10.certificatCurent = _this10.certificate.find(function (c) {
          return c.id === id;
        }) || _this10.certificatCurent;
      })["catch"](function (err) {
        _this10.eroareModal = _this10.mesajEroare(err, 'Lista nu a putut fi reîncărcată');
      });
    },
    aboneaza: function aboneaza() {
      ...
      this.$http.post('/anaf-certificate/abonare', {
        ...
        _this11.eroare = _this11.mesajEroare(err, 'Adresa nu a putut fi înscrisă');
      });
    },
...
/* harmony import */ var _libs_flux__WEBPACK_IMPORTED_MODULE_25__ = __webpack_require__(/*! @/libs/flux */ "./resources/js/src/libs/flux.js");
  ...
  methods: {
    ...
    trimiteLaAnaf: function trimiteLaAnaf(declaratii) {
      ...
      var urmatoarea = function urmatoarea(i) {
        if (i >= declaratii.length) {
          ...
          _this12.notifica("".concat(declaratii.length - esuate.length, " din ").concat(declaratii.length, " depuse"), esuate.length ? 'warning' : 'success');

          return null;
        }

        var declaratie = declaratii[i];
        _this12.depunereMesaj = "".concat(declaratie.tip, " (").concat(i + 1, " din ").concat(declaratii.length, ")");
        return _this12.$http.post("/declaratii/".concat(declaratie.id, "/depune"))["catch"](function (err) {
          esuate.push("".concat(declaratie.tip, " ").concat(declaratie.cui, ": ").concat(_this12.mesajEroare(err, 'depunere eșuată')));
        }).then(function () {
          return urmatoarea(i + 1);
        });
      };
    ...
    actiune: function actiune(declaratie, tip) {
      ...
      this.$http.post("/declaratii/".concat(declaratie.id, "/").concat(tip)).then(function () {
        _this13.notifica(mesaje[tip], 'success');

        _this13.incarcaLista();
      })["catch"](function (err) {
        _this13.eroare = _this13.mesajEroare(err, 'Operația a eșuat');
    ...
    verificaRecipise: function verificaRecipise() {
      ...
      this.cereRecipisele().then(function (rezultat) {
        _this14.ultimaDescarcare = _this14.acum();
        _this14.ultimaDescarcareNumar = rezultat.descarcate || 0;
        window.localStorage.setItem('declaratii_recipise_ultima', JSON.stringify({
          la: _this14.ultimaDescarcare,
          descarcate: _this14.ultimaDescarcareNumar
        }));

        if (!tacut || rezultat.descarcate > 0) {
          _this14.notifica("Verificate: ".concat(rezultat.verificate, ", recipise desc\u0103rcate: ").concat(rezultat.descarcate), rezultat.descarcate > 0 ? 'success' : 'info');
        }

        if (rezultat.erori && rezultat.erori.length) {
          _this14.eroare = rezultat.erori.join(' | ');
        }

        _this14.incarcaLista(); // Recipisele venite acum se pot aduna într-un singur fișier de tipărit.


        var aduse = rezultat.descarcate_id || [];

        if (_this14.tiparireRecipise && aduse.length) {
          return _this14.descarcaPentruTiparire(aduse, 'recipisa');
        }

        return null;
      })["catch"](function (err) {
        _this14.eroare = _this14.mesajEroare(err, 'Descărcarea recipiselor a eșuat');
      })["finally"](function () {
        _this14.recipiseInCurs = false;
        _this14.mersul = '';
        _this14.cercetate = 0;
        _this14.deCercetat = 0;
      });
    },
    ...
    cereRecipisele: function cereRecipisele() {
      ...
      if (!Object(_libs_flux__WEBPACK_IMPORTED_MODULE_25__["areFlux"])()) {
      ...
      return Object(_libs_flux__WEBPACK_IMPORTED_MODULE_25__["default"])('declaratii/recipise/flux', function (pas) {
        if (pas.tip === 'inceput') {
          _this15.deCercetat = pas.total;
          _this15.cercetate = 0;
          if (pas.total) _this15.mersul = "0 din ".concat(pas.total, " declara\u021Bii cercetate...");
          return;
        }

        if (pas.tip === 'pas') {
          _this15.cercetate = pas.facute;
          var care = pas.ce ? " \u2014 ".concat(pas.ce) : '';
          var adusa = pas.adus ? ' (recipisă adusă)' : '';
          _this15.mersul = "".concat(pas.facute, " din ").concat(pas.total, " declara\u021Bii cercetate").concat(care).concat(adusa);
          return;
        }
    ...
    sterge: function sterge(declaratie) {
      ...
      this.$bvModal.msgBoxConfirm("\u0218terge\u021Bi declara\u021Bia ".concat(declaratie.tip, " pentru CUI ").concat(declaratie.cui, "?"), {
        ...
        _this16.$http["delete"]("/declaratii/".concat(declaratie.id)).then(function () {
          _this16.notifica('Declarația a fost ștearsă', 'success');

          _this16.incarcaLista();
        })["catch"](function (err) {
          _this16.eroare = _this16.mesajEroare(err, 'Ștergerea a eșuat');
        });
      });
    },
    mesajEroare: function mesajEroare(err, implicit) {
    ...
    explicaEroarea: function explicaEroarea(declaratie) {
      ...
      fetch(adresa, {
        ...
        return _this17.citesteFluxul(raspuns.body.getReader());
      })["catch"](function (err) {
        // Eșecul se arată în fereastră, nu în spatele ei: altfel utilizatorul
        // vede doar o fereastră goală și nu știe ce s-a întâmplat.
        _this17.explicatieInCurs = false;
        _this17.explicatieEroare = "Explica\u021Bia nu a putut fi ob\u021Binut\u0103 (".concat(err.message, ").");
      });
    },
    ...
    citesteFluxul: function citesteFluxul(cititor) {
      ...
      var urmatorul = function urmatorul() {
        return cititor.read().then(function (_ref4) {
          var done = _ref4.done,
              value = _ref4.value;

          if (done) {
            _this18.explicatieInCurs = false;
            return null;
          }
...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    loadMessages: function loadMessages() {
      ...
      return this.$http.get('/spv', {
        ...
        if (response.data && response.data.success) {
          ...
          return _this10.aduceDocumentele(payload, arata);
        }
    ...
    aduceDocumentele: function aduceDocumentele(payload, arata) {
      ...
      var intrebare = this.listaCif.length === 1 ? "?cif=".concat(encodeURIComponent(this.listaCif[0])) : ''; // Browserele fără fetch cu flux rămân pe calea dinainte: loturi, fără
      ...
      if (!Object(_libs_flux__WEBPACK_IMPORTED_MODULE_19__["areFlux"])()) {
        return this.$http.get('/spv/descarca-lipsa', {
          params: this.listaCif.length === 1 ? {
            cif: this.listaCif[0]
          } : {}
        }).then(function (raspuns) {
          return _this11.aduceRestul(payload, raspuns.data && raspuns.data.descarcare || {}, arata);
        });
      }
      ...
      return Object(_libs_flux__WEBPACK_IMPORTED_MODULE_19__["default"])("spv/descarca-lipsa/flux".concat(intrebare), function (pas) {
        if (pas.tip === 'inceput') {
          _this11.deAdus = pas.total;
          _this11.aduse = 0;
          if (arata && pas.total) _this11.mersul = "0 din ".concat(pas.total, " documente aduse...");
          return;
        }

        if (pas.tip === 'pas') {
          _this11.aduse = pas.facute;

          if (arata) {
            var care = pas.ce ? " \u2014 ".concat(pas.ce) : '';
            var cazut = pas.reusit ? '' : ' (nu s-a putut aduce)';
            _this11.mersul = "".concat(pas.facute, " din ").concat(pas.total, " documente aduse").concat(care).concat(cazut);
          }
        ...
        if (pas.tip === 'gata') {
          ...
          if (Array.isArray(pas.mesaje)) _this11.messages = pas.mesaje;
        }
      }).then(function () {
        if (arata) _this11.infoDescarcare = _this11.rezumatCitire(payload, rezultat);
      })["catch"](function (err) {
        _this11.error = "Aducerea documentelor s-a oprit (".concat(err.message, ").");
      })["finally"](function () {
        _this11.mersul = '';
        _this11.aduse = 0;
        _this11.deAdus = 0;
    ...
    aduceRestul: function aduceRestul(payload, descarcare, arata) {
      ...
      var incaUnLot = function incaUnLot() {
        ...
        if (arata) {
          _this12.infoDescarcare = "".concat(adunate.descarcate, " documente aduse, ") + "".concat(adunate.ramase, " de adus. V\u0103 rug\u0103m a\u0219tepta\u021Bi...");
        }

        return _this12.$http.get('/spv/descarca-lipsa', {
          params: params
        }).then(function (response) {
          var _adunate$erori;
          ...
          if (response.data && response.data.data && Array.isArray(response.data.data.mesaje)) {
            _this12.messages = response.data.data.mesaje;
          }
      ...
      return incaUnLot().then(function () {
        if (arata) _this12.infoDescarcare = _this12.rezumatCitire(payload, adunate);
      })["catch"](function (err) {
        _this12.error = err.response && err.response.data && err.response.data.message ? err.response.data.message : 'Aducerea documentelor rămase s-a oprit';
      });
    },
    ...
    reglaCronometrul: function reglaCronometrul() {
      ...
      this.cronometru = window.setInterval(function () {
        if (!_this13.loading) _this13.loadMessages(true);
      }, minute * 60 * 1000);
    }
  }
});
  ...
  methods: {
    ...
    schimbaUzul: function schimbaUzul(societate) {
      ...
      this.$http.put("/anaf-societati/".concat(societate.id), {
        ...
        _this2.info = scoasa ? "".concat(societate.denumire || societate.cif, " a fost scoas\u0103 din uz \u0219i va fi ignorat\u0103 peste tot.") : "".concat(societate.denumire || societate.cif, " este iar \xEEn lucru.");
        return _this2.incarcaLista();
      })["catch"](function (err) {
        _this2.eroare = _this2.mesajEroare(err, 'Starea entității nu a putut fi schimbată');
      })["finally"](function () {
        _this2.schimbaStarea = null;
      });
    },
    incarcaLista: function incarcaLista() {
      var _this3 = this;
      ...
      return this.$http.get('/anaf-societati', {
        ...
        _this3.societati = raspuns.data.data || [];
      })["catch"](function (err) {
        _this3.eroare = _this3.mesajEroare(err, 'Nu s-a putut încărca lista de entități');
      })["finally"](function () {
        _this3.listaInCurs = false;
      });
    },
    sincronizeaza: function sincronizeaza() {
      ...
      this.$http.post('/anaf-societati/sincronizeaza').then(function (raspuns) {
        ...
        _this4.info = parti.join(', ');

        _this4.incarcaLista();
      })["catch"](function (err) {
        _this4.eroare = _this4.mesajEroare(err, 'Sincronizarea a eșuat');
      })["finally"](function () {
    ...
    solicita: function solicita() {
      ...
      this.ceEDeja().then(function () {
        return _this5.trimiteSolicitari(cifuri);
      }).then(function (rezumat) {
        return _this5.descarcaMesajeNoi(rezumat);
      }).then(function (rezumat) {
        return _this5.preiaRaspunsurile(rezumat);
      }).then(function (rezumat) {
        var parti = [];
        ...
        _this5.info = parti.length ? parti.join(', ') : 'Nu era nimic de solicitat.';
        if (_this5.erori.length) _this5.eroare = _this5.erori.join(' | ');

        _this5.incarcaLista();
      })["catch"](function (err) {
        _this5.eroare = _this5.mesajEroare(err, 'Solicitarea a eșuat');
      })["finally"](function () {
        _this5.solicitareInCurs = false;
        _this5.progres = null;
      });
    },
    ...
    ceEDeja: function ceEDeja() {
      ...
      return this.$http.get('/spv', {
        ...
        if (d.erori && d.erori.length) (_this6$erori = _this6.erori).push.apply(_this6$erori, Object(C_dianasoft_serviciiweb_web_node_modules_babel_runtime_helpers_esm_toConsumableArray_js__WEBPACK_IMPORTED_MODULE_1__["default"])(d.erori));
        _this6.progres = {
          pas: 'mesaje',
          facut: 1,
          total: 1,
        ...
        return _this6.$http.post("/spv/solicitari/preia?zile=1&limita=".concat(_this6.RASPUNSURI_PE_LOT));
      }).then(function () {
        ...
        _this6.erori.push(_this6.mesajEroare(err, 'Documentele venite deja nu au putut fi citite'));
      });
    },
    ...
    trimiteSolicitari: function trimiteSolicitari(cifuri) {
      ...
      var lot = function lot(i) {
        ...
        var acum = cifuri.slice(i, i + _this7.FIRME_PE_LOT);
        _this7.progres = {
          pas: 'solicitari',
          facut: i,
          total: cifuri.length,
        ...
        return _this7.$http.post('/anaf-societati/solicita', {
          cif: acum,
          reinterpreteaza: i === 0
        }).then(function (raspuns) {
          var _this7$erori;
          ...
          if (r.erori && r.erori.length) (_this7$erori = _this7.erori).push.apply(_this7$erori, Object(C_dianasoft_serviciiweb_web_node_modules_babel_runtime_helpers_esm_toConsumableArray_js__WEBPACK_IMPORTED_MODULE_1__["default"])(r.erori));
          return lot(i + _this7.FIRME_PE_LOT);
        });
      };
    ...
    descarcaMesajeNoi: function descarcaMesajeNoi(rezumatul) {
      ...
      var runda = function runda(trecute) {
        ...
        return _this8.$http.get('/spv', {
          ...
          if (d.erori && d.erori.length) (_this8$erori = _this8.erori).push.apply(_this8$erori, Object(C_dianasoft_serviciiweb_web_node_modules_babel_runtime_helpers_esm_toConsumableArray_js__WEBPACK_IMPORTED_MODULE_1__["default"])(d.erori)); // Se oprește când nu mai rămâne nimic sau când o rundă n-a mai adus nimic.

          if (!d.ramase || !d.descarcate || trecute >= _this8.RUNDE_MAXIME) {
            return rezumat;
          }
    ...
    preiaRaspunsurile: function preiaRaspunsurile(rezumatul) {
      ...
      var runda = function runda(trecute) {
        ...
        return _this9.$http.post("/spv/solicitari/preia?limita=".concat(_this9.RASPUNSURI_PE_LOT)).then(function (raspuns) {
          var _this9$erori;

          var r = raspuns.data.data || {};
          rezumat.preluate += r.preluate || 0;
          if (r.erori && r.erori.length) (_this9$erori = _this9.erori).push.apply(_this9$erori, Object(C_dianasoft_serviciiweb_web_node_modules_babel_runtime_helpers_esm_toConsumableArray_js__WEBPACK_IMPORTED_MODULE_1__["default"])(r.erori)); // Cele fără răspuns încă nu se mai așteaptă: vin la o rulare viitoare.

          if (!r.preluate || trecute >= _this9.RUNDE_MAXIME) {
            return rezumat;
          }
  ...
  methods: {
      ...
      return this.cereRaspunsurile().then(function (rezultat) {
        ...
        return _this7.incarcaLista().then(function () {
          ...
          var noi = _this7.solicitari.filter(function (s) {
            return _this7.areDocument(s) && inainte.indexOf(s.id) === -1;
          }).map(function (s) {
            return s.id;
          });
    ...
    cereRaspunsurile: function cereRaspunsurile() {
      ...
      return Object(_libs_flux__WEBPACK_IMPORTED_MODULE_16__["default"])('spv/solicitari/preia/flux', function (pas) {
        if (pas.tip === 'inceput') {
          _this8.deAdus = pas.total;
          _this8.aduse = 0;
          if (pas.total) _this8.mersul = "0 din ".concat(pas.total, " r\u0103spunsuri aduse...");
          return;
        }

        if (pas.tip === 'pas') {
          _this8.aduse = pas.facute;
          var care = pas.ce ? " \u2014 ".concat(pas.ce) : '';
          var cazut = pas.reusit ? '' : ' (nu s-a putut aduce)';
          _this8.mersul = "".concat(pas.facute, " din ").concat(pas.total, " r\u0103spunsuri aduse").concat(care).concat(cazut);
          return;
        }
    ...
    tipareste: function tipareste(ids) {
      ...
      return this.$http.post('/spv/solicitari/tipareste', {
        ...
        _this9.notifica("".concat(data.data.documente, " documente trimise la \u201E").concat(data.data.imprimanta, "\u201D"), 'success');
      })["catch"](function (err) {
        var motiv = _this9.mesajEroare(err, 'programul local nu a răspuns.');

        _this9.eroare = "R\u0103spunsurile au fost preluate, dar tip\u0103rirea nu a reu\u0219it: ".concat(motiv);
      });
    },
    ...
    reglaCronometrul: function reglaCronometrul() {
      ...
      this.cronometru = window.setInterval(function () {
        ...
        if (!_this10.preiaInCurs && _this10.inAsteptare) {
          _this10.preia(true);
        }
      }, minute * 60 * 1000);
    },
    sterge: function sterge(solicitare) {
      ...
      this.$http["delete"]("/spv/solicitari/".concat(solicitare.id)).then(function () {
        _this11.notifica('Solicitarea a fost ștearsă', 'success');
...
function adresa(cale) {
  ...
  var baza = (window.api_url || _axios__WEBPACK_IMPORTED_MODULE_7__["default"].defaults.baseURL || '').replace(/\/+$/, '');
...
function citesteFluxul(cale, laFiecarePas) {
  var antete = _axios__WEBPACK_IMPORTED_MODULE_7__["default"].defaults.headers.common;
