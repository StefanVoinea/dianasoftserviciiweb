...
<td align ="center" width="50%">
 ...
 <td width=50%>
        ...
        Seria: <strong>{{$vanzare->seria}}</strong> Număr: <strong>{{$vanzare->numar}}</strong>Cota TVA: <strong>19 % </strong> <br>
...
<hr>
 <div >
     ...
     <table class="table table-condesed text-sm" width=100%>      
      ...
      @if($vanzare->contract["tip_contract"]=="Servicii funerare")
        ...
        @if($vanzare->numerar||$vanzare->card)
          {{"Achitat cu ".$vanzare->tip_incasare." nr ".$vanzare->numar_incasare}}
        @endif       
    ...
    <hr>
    <div class="text-sm" >
    Întocmită de : {{Auth::user()->name .' '.datasioracurenta()}} <br>
...
<div class="container">
      ...
      <div>
      <h3>
      Emitent:<strong> {{$company->denumire}}</strong>
      </h3>
       Reg.com.:<strong> {{$company->regcom}}</strong>  C.U.I.: <strong>{{$company->cui}}</strong>
      </div>
      <div>
    ...
    <table class="table" width="80%">
    	<tr>
    		<td width=100% align="center">
	    		<h3><strong>DISPOZIȚIE DE PLATĂ</strong> Număr: <strong>____________</strong>
				Data:   <strong>{{dateFormatAfisare($incasare->data_document)}}</strong></h3>
...
<table class="table" width="80%">
	<tr>
    		<td >
				...
				Beneficiarul sumei: <strong>{{ $incasare->partener }}</strong>  C.N.P.: <strong>{{ $incasare->cui }}</strong>   <br>
				Suma : <strong>{{number_format(abs($incasare->suma),2).' LEI adica '.nr2litere(number_format(abs($incasare->suma),2))}}</strong><br>
				Scopul plății:   <strong>{{'Restituire contravaloare factura nr.'.$vanzare->numar.'/'.dateFormatAfisare($vanzare->data)}}</strong> <br>
