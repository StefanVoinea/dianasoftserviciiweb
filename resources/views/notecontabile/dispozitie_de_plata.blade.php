    ...
    <table class="table" width="80%">
    	<tr>
    		<td width=100% align="center">
	    		<h3><strong>DISPOZITIE DE PLATA CATRE CASIERIE</strong> Numar: <strong>{{$dispozitie->nr_doc}}</strong>
...
<table class="table" width="80%">
	<tr>
    		<td >
				...
				Beneficiarul sumei: <strong>{{ $dispozitie->partener }}</strong>  C.N.P.: <strong>{{ $dispozitie->partenerul["cui"] }}</strong>   <br>
				Suma : <strong>{{number_format(abs($dispozitie->suma),2).' LEI adica '.nr2litere(number_format(abs($dispozitie->suma),2))}}</strong><br>
				Scopul plății:   <strong>{{$dispozitie->expl}}</strong> <br>
