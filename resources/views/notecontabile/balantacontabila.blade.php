...
@section ('content')
      ...
      <table class="text-sm table table-condesed" border="1" style="font-family: 'Times New Roman', Times, serif; font-size: 15px; border-collapse: collapse;" width=100%>
            <thead>
            <tr >
            ...
            <tr style=" page-break-inside: avoid !important;">
              ...
              <th align ="center"  width="8%">
                    ...
                    @foreach($balanta->groupBy("grupa") as $grupa)
                      @foreach($grupa->groupBy("cont_sintetic",SORT_STRING) as $contSintetic)
                          @foreach($contSintetic->sortBy('cont',SORT_STRING) as $cont)
                            <tr style="height:30px; page-break-inside: avoid !important;"> 
                             <td align="left" width="5%">
                              {{$cont->cont }} 
                             </td>
                             <td align="left" style="font-size: 12px;" width="10%">
                              {{$cont->denumire }} 
                             </td>
                     ...
                     <th align="left"  colspan="2" width="15%">
                       TOTAL CLASA {{($contSintetic->values()[0]->cont_sintetic)}}
                       </th>
                      <!--  <th align="right" width="8%">
                          {{ number_format($contSintetic->sum("soldiniD"),2)}}
                      </th>
                      <th align="right" width="8%">
                          {{ number_format($contSintetic->sum("soldiniC"),2)}}
                       </th> -->
                     
                        <th align="right" width="8%">
                          {{ number_format($contSintetic->sum("rulajAntD"),2)}}
                      </th>
                      <th align="right" width="8%">
                          {{ number_format($contSintetic->sum("rulajAntC"),2)}}
                       </th>
                     <th align="right" width="8%">
                          {{ number_format($contSintetic->sum("rulajD"),2)}}
                      </th>
                      <th align="right" width="8%">
                          {{ number_format($contSintetic->sum("rulajC"),2)}}
                       </th>
                       <th align="right" width="8%">
                          {{ number_format($contSintetic->sum("rulajTotalD"),2)}}
                      </th>
                      <th align="right" width="8%">
                          {{ number_format($contSintetic->sum("rulajTotalC"),2)}}
                       </th>
                        <th align="right" width="8%">
                          {{ number_format($contSintetic->sum("soldFinalD"),2)}}
                      </th>
                      <th align="right" width="8%">
                          {{ number_format($contSintetic->sum("soldFinalC"),2)}}
                       </th>
