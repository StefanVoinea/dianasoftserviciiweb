      ...
      >
        ...
        <div class="chat-navbar">
          <header class="chat-header">
            ...
            <div class="d-flex align-items-center">
              ...
              <b-avatar
                size="36"
                :src="activeChat.contact.link_poza"
        ...
        >
          <chat-log
            ...
            :profile-user-avatar="profileUserDataMinimal.link_poza"
          />
        </vue-perfect-scrollbar>
...
export default {
  ...
  setup() {
    ...
    const fetchChatAndContacts = () => {
      store.dispatch('app-chat/fetchChatsAndContacts')
        .then(response => {
          ...
          contacts.value = JSON.parse(response.data.contacts)
    ...
    const sendMessage = () => {
      ...
      store.dispatch('app-chat/sendMessage', payload)
        .then(response => {
          ...
          let contact = chatsContacts.value.find(c => c.id === response.data.contact.id)
          
          if (contact === undefined) {
             
            activeChat.value = { chat, contact: activeChat.value.contact }
            activeChat.value.contact.lastMessage= newMessageData
            activeChat.value.contact.unseenMsgs= 0
            //activeChat.value.contact.chat=new Array() 
          ...
          // contact.lastMessage = newMessageData
    ...
    const showUserProfileSidebar = () => {
      profileUserData.value= JSON.parse(store.state.app.user)
